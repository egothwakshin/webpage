...
public class file10 {
	...
	public static void main(String[] args) throws Exception {
		...
		//io + nio 복사
		File f1 = new File(url);	//원본파일
		File f2 = new File(url2);	//복사할 파일		
		Files.copy(f1.toPath(), f2.toPath(), StandardCopyOption.REPLACE_EXISTING);
		
		
		//io + nio 복사2  rwx - (r:읽기 w:쓰기 x:실행)
		//RandomAccessFile : 해당파일 권한에 맞춰 쓰기, 읽기를 설정하는 클래스
		RandomAccessFile f3 = new RandomAccessFile(url, "r");
		RandomAccessFile f4 = new RandomAccessFile(url2, "rw");
		
		//FileChannel : nio 클래스 (파일읽기, 쓰기, 맵핑)
		FileChannel fc = f3.getChannel();
		System.out.println(fc.size());		//원본파일 용량 확인
		FileChannel fc2 = f4.getChannel();
		
		fc.transferTo(0, fc.size(), fc2);	//transferTo 데이터 '0byte~해당크기'만큼 내용 복사
	}
}
